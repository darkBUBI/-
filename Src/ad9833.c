...
//***************************
	...
	#define PIN_CS		GPIO_PIN_1  //数字电位器片选

//****************************************************************

	#define FSYNC_0()		HAL_GPIO_WritePin(PORT_FSYNC, PIN_FSYNC,0)//GPIO_ResetBits(PORT_FSYNC, PIN_FSYNC)
	#define FSYNC_1()		HAL_GPIO_WritePin(PORT_FSYNC, PIN_FSYNC,1)//GPIO_SetBits(PORT_FSYNC, PIN_FSYNC)

	#define SCK_0()		HAL_GPIO_WritePin(PORT_SCK, PIN_SCK,0)//GPIO_ResetBits(PORT_SCK, PIN_SCK)
	#define SCK_1()		HAL_GPIO_WritePin(PORT_SCK, PIN_SCK,1)//GPIO_SetBits(PORT_SCK, PIN_SCK)

	#define DAT_0()		HAL_GPIO_WritePin(PORT_DAT, PIN_DAT,0)//GPIO_ResetBits(PORT_DAT, PIN_DAT)
	#define DAT_1()		HAL_GPIO_WritePin(PORT_DAT, PIN_DAT,1)//GPIO_SetBits(PORT_DAT, PIN_DAT)

	#define CS_0()		HAL_GPIO_WritePin(PORT_CS, PIN_CS,0)//GPIO_ResetBits(PORT_DAT, PIN_CS)
	#define CS_1()		HAL_GPIO_WritePin(PORT_CS, PIN_CS,1)//GPIO_SetBits(PORT_DAT, PIN_CS)
...
//   CS_1();
	...
	FSYNC_0();
	//AD9833_Delay();
	temp =0x1100|amp;//0001 0001 1111 1111
	for(i = 0; i < 16; i++)
	{
		if (temp & 0x8000)
			DAT_1();
